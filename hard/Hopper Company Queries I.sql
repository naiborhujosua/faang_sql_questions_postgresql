...
tm as 
(
    select month,left(("2019-12-01" + interval "1 month"),7) ym from t
)
select month, 
    count(distinct Drivers.driver_id) active_drivers,
    count(distinct AcceptedRides.ride_id) accepted_rides 
from tm left join Drivers on to_char(Drivers.join_date,'YYYY-MM') <= t.ym
    left join Rides on to_char(Rides.requested_at,'YYYY-MM') = ym
    left join AcceptedRides 
    using(ride_id)
group by month
